...
class ForumMixin(object):
    def get_context_data(self, **kwargs):
        """
        Mixin for forum active menu
        """
        context = super(ForumMixin, self).get_context_data(**kwargs)
        context.update({'active': 'forum'})
        return context
...
class CategoryListView(ForumMixin, ListView):
    model = Category
...
class TopicDetailView(ForumMixin, DetailView):
    model = Topic


class TopicCreateView(ForumMixin, FormView):
...
class PostCreateView(ForumMixin, FormView):
